...
namespace RRCG
{
    public class CircuitDescriptorRewriter : CSharpSyntaxRewriter
    {
        ...
        public SyntaxNode VisitClassDeclarationRoot(ClassDeclarationSyntax node)
        {
            ...
            // Rewrite members into Member Variables as necessary
            ...
            foreach (var member in allMembers.ToList()) // shallow copy
            {
                ...
                if (member is FieldDeclarationSyntax field)
                    VisitPotentialMemberVariable(field, allMembers, attributeName, attributeArgs, variableKind);
                else if (member is PropertyDeclarationSyntax property)
                    VisitPotentialMemberVariable(property, allMembers, attributeName, attributeArgs, variableKind);
                else
        ...
        void VisitPotentialMemberVariable(FieldDeclarationSyntax field,
                                  ...
                                  MemberAccessExpressionSyntax variableKindAccessor)
        {
                                ...
                                EqualsValueClause(
                                    ...
                                    .WithArgumentList(
                                        SyntaxUtils.ArgumentList(
                                            ...
                                            homeValueExpression,
                                            variableKindAccessor))))))).WithModifiers(varFieldModifiers));
                                  ...
                                  MemberAccessExpressionSyntax variableKindAccessor)
        {
            ...
            // Setup NamedVariable type
            // Need to re-parse so the root is TypeSyntax, see VisitIdentifierName
            var rewrittenType = (TypeSyntax)Visit(ParseTypeName(property.Type.ToString()));
            var namedVariableType = GenericName("NamedVariable")
                                        .WithTypeArgumentList(
                                            SyntaxUtils.TypeArgumentList(rewrittenType));
